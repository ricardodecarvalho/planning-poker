...
const PokerTable: React.FC<PokerTableProps> = ({ votingStatus, isShowVotes, users, votingSystemType }) => {
        ...
        <TableContainer>
            <Table>
                <CenterContent>
                    {isShowVotes ? (
                        <>
                            <div>{isNumericSystem ? t('pokerRoom.average') : t('pokerRoom.mostCommon')}</div>
                            <AverageValue>{displayValue}</AverageValue>
                        </>
                    ) : (
                        <div style={{ opacity: 0.7 }}>
                            {t('pokerRoom.waitingForVote')}
                        </div>
                    )}
                </CenterContent>
            </Table>
            ...
            {seatedUsers.map((user, index) => {
                ...
                const distance = (baseA * baseB) / Math.sqrt(Math.pow(baseB * Math.cos(angleRad), 2) + Math.pow(baseA * Math.sin(angleRad), 2));
